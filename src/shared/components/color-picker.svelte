...
function updateCsPicker() {
    let csPicker = document.querySelector<HTMLDivElement>('#colorsquare-picker')
...
function updateHuePicker() {
    let huePicker = document.querySelector<HTMLDivElement>('#hue-picker')
    let xPercentage = h * 100
    huePicker.style.left = xPercentage + '%'
}

function updateAlphaPicker() {
    let alphaPicker = document.querySelector<HTMLDivElement>('#alpha-picker')
...
function csDown(event) {
    ...
    let xPercentage: any = ((event.offsetX + 1) / 240) * 100
    let yPercentage: any = ((event.offsetY + 1) / 160) * 100
    yPercentage = yPercentage.toFixed(2)
    xPercentage = xPercentage.toFixed(2)
    let picker = document.querySelector<HTMLDivElement>('#colorsquare-picker')
    picker.style.top = yPercentage + '%'
    picker.style.left = xPercentage + '%'
    s = xPercentage / 100
    v = 1 - yPercentage / 100
...
function csDownTouch(event) {
    ...
    let xPercentage: any = ((offsetX + 1) / 240) * 100
    let yPercentage: any = ((offsetY + 1) / 160) * 100
    yPercentage = yPercentage.toFixed(2)
    xPercentage = xPercentage.toFixed(2)
    let picker = document.querySelector<HTMLDivElement>('#colorsquare-picker')
    picker.style.top = yPercentage + '%'
...
function hueDown(event) {
    tracked = event.currentTarget
    let xPercentage: any = ((event.offsetX - 9) / 220) * 100
    xPercentage = xPercentage.toFixed(2)
    let picker = document.querySelector<HTMLDivElement>('#hue-picker')
    picker.style.left = xPercentage + '%'
    h = xPercentage / 100
    hueChange()
}
...
function hueDownTouch(event) {
    ...
    let rect = event.target.getBoundingClientRect()
    let offsetX = event.targetTouches[0].clientX - rect.left
    let xPercentage: any = ((offsetX - 9) / 220) * 100
    xPercentage = xPercentage.toFixed(2)
    let picker = document.querySelector<HTMLDivElement>('#hue-picker')
    picker.style.left = xPercentage + '%'
    h = xPercentage / 100
...
function hueChange() {
    ...
    let colorsquare = document.querySelector<HTMLDivElement>('.colorsquare')
...
function colorChange() {
    ...
    b = rgb[2]
    hexValue = RGBAToHex()
    let pickedColor = document.querySelector<HTMLDivElement>('.color-picked')
...
function alphaDown(event) {
    ...
    let xPercentage: any = ((event.offsetX - 9) / 220) * 100
    xPercentage = xPercentage.toFixed(2)
    let picker = document.querySelector<HTMLDivElement>('#alpha-picker')
    picker.style.left = xPercentage + '%'
    a = xPercentage / 100
    colorChange()
}
